...
public class CutsceneDirector : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (CutscenePlaying && Input.PrimaryButton == ParseInputs.ButtonState.Pressed)
        {
    ...
    void PlayTimeline(string Name)
    {
        ...
        TL.playableAsset = Resources.Load<PlayableAsset>("Timeline/" + Name);
        if(TL.playableAsset != null)
        {
            TL.Play();
        } else
        {
    ...
    void EndTimeline(PlayableDirector pd)
    {
        //Handles finishing a timeline event

        //If there is another line of dialogue after the cutscene, send it to be displayed
        if(InkStory.currentText != "")
        {
            CanvasController.CC.ToggleDialogue(); //Assume that using a Timeline hid the dialogue
    ...
    IEnumerator EndDelay()
    {
        ...
        //Allow player to continue shortly after the last click so their weapon doesn't go off
        PlayerRef.PlayerCanMove = true;
    }
    ...
    public void ContinueDialogue()
    {
        ...
        } else
        {
            ...
            } else
            {
                ...
                if (CanvasController.CC.DialogueShowing())
                {
                    CanvasController.CC.ToggleDialogue();
                }
